...
export async function buildClient({
  ...
  // - The main client package exports map can be simplified:
...
function getDefaultOutdir(outputDir: string): string {
  if (outputDir.endsWith(path.normalize(path.join('node_modules', clientPkg.name)))) {
...
export async function generateClient(options: GenerateClientOptions): Promise<void> {
  ...
  if (isWasmEngineSupported(provider)) {
    ...
    // necessarily inside the bundled Prisma CLI (`prisma-lossless generate` can also run
    // from a standalone or programmatic invocation). This means we can only rely
    // on what's shipped in the client package here, and we have to
...
async function getGenerationDirs({
  ...
  let userRuntimeImport = isCustomOutput ? './runtime' : `${clientPkg.name}/runtime`
...
async function verifyOutputDirectory(directory: string, datamodel: string, schemaPath: string) {
  ...
  if (name === clientPkg.name) {
    ...
    message.push('This package is used by `prisma-lossless generate` and overwriting its content is dangerous.')
    message.push('')
    ...
    message.push(`Imports from \`${clientPkg.name}\` will be automatically forwarded to \`.prisma/client\``)
