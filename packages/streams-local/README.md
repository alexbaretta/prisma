...
the published `@prisma-lossless/streams-local` runtime.
...
- `@prisma-lossless/streams-local/internal/daemon` (internal Prisma CLI plumbing)
...
1. Start a named local server from `@prisma-lossless/streams-local`.
...
```ts
import { startLocalDurableStreamsServer } from "@prisma-lossless/streams-local";
...
const child = fork(require.resolve("@prisma-lossless/streams-local/internal/daemon"), [
  "--name",
  "default",
  "--port",
  "0",
