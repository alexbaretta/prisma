...
`@prisma-lossless/studio-core` is the embeddable Prisma Studio package.
...
} from "@prisma-lossless/studio-core/ui";
import "@prisma-lossless/studio-core/ui/index.css";
...
import { isStudioLlmResponse } from "@prisma-lossless/studio-core/data";
...
- import `@prisma-lossless/studio-core/ui/index.css` exactly once
...
```ts
import { serializeError, type StudioBFFRequest } from "@prisma-lossless/studio-core/data/bff";
...
- starts Prisma Postgres dev (`ppg-dev`) programmatically via `@prisma-lossless/dev`
...
`@prisma-lossless/dev` can resolve its PGlite runtime assets (WASM/data/extensions)
