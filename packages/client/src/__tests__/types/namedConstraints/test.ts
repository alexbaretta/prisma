...
async function main() {
  ...
  const result2: { key1: string; key2: number } | null = await prisma.atAtId.findUnique({
    where: {
      key1_key2: {
        key1: 'data',
        key2: 2,
  ...
  const result4: { key1: string; key2: number } | null = await prisma.atAtUnique.findUnique({
    where: {
      key1_key2: {
        key1: 'data',
        key2: 2,
