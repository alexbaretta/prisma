...
vi.mock('@prisma-lossless/migrate', () => ({
  MigrateDev: {
    new: () => ({
...
describe('Bootstrap command — help and validation', () => {
  test('shows help with --help flag', async () => {
    ...
    expect(result).toContain('prisma-lossless bootstrap')
  })
...
describe('Bootstrap command — new project flow', () => {
  test('runs init when user declines template, then links', async () => {
    ...
    fs.mkdirSync(path.join(tmpDir, 'node_modules', '@prisma-lossless', 'cli'), { recursive: true })
    fs.mkdirSync(path.join(tmpDir, 'node_modules', '@prisma-lossless', 'client'), { recursive: true })
...
describe('Bootstrap command — deps gate', () => {
  test('prompts to install missing deps and continues when accepted', async () => {
    ...
    const output = result as string
    expect(output).toContain('Bootstrap completed')
    expect(addDependencies).toHaveBeenCalledWith(tmpDir, ['@prisma-lossless/client'])
    expect(addDevDependencies).toHaveBeenCalledWith(tmpDir, ['dotenv', '@prisma-lossless/cli'])
  })
