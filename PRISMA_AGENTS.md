...
- **Testing & databases**: `TESTING.md` covers Jest/Vitest usage. Most suites run as `pnpm --filter @prisma/<pkg> test <pattern>`. DB-backed tests expect `.db.env` and Docker services from `docker/docker-compose.yml` (`docker compose up -d`). Client functional tests sit in `packages/client/tests/functional`—run them via `pnpm --filter @prisma-lossless/client test:functional` (with typechecking) or `pnpm --filter @prisma-lossless/client test:functional:code` (code only); `helpers/functional-test/run-tests.ts` documents CLI flags to target providers, drivers, etc. Client e2e suites require a fresh `pnpm build` at repo root, then `pnpm --filter @prisma-lossless/client test:e2e --verbose --runInBand`. The legacy `pnpm --filter @prisma-lossless/client test` command primarily runs the older Jest unit tests plus `tsd` type checks. Migrate CLI suites live in `packages/migrate/src/__tests__`, the CLI runs both Jest (legacy suites) and Vitest (new subcommand coverage) via `pnpm --filter prisma test`, and end-to-end coverage lives in `packages/integration-tests`.
...
- **Client functional tests structure**:
  ...
  - Run specific adapter: `pnpm --filter @prisma-lossless/client test:functional:code --adapter js_pg <pattern>` (adapters: `js_pg`, `js_neon`, `js_libsql`, `js_planetscale`, `js_d1`, `js_better_sqlite3`, `js_mssql`, `js_mariadb`, `js_pg_cockroachdb`).
...
- **Adding PrismaClient constructor options**:
  ...
  - Use `@prisma-lossless/ts-builders` for generating TypeScript type declarations.
...
- **SQL Commenter packages**:
  - `@prisma-lossless/sqlcommenter`: Core types (`SqlCommenterPlugin`, `SqlCommenterContext`, `SqlCommenterQueryInfo`, `SqlCommenterTags`) for building sqlcommenter plugins.
  - `@prisma-lossless/sqlcommenter-query-tags`: AsyncLocalStorage-based plugin for adding ad-hoc tags via `withQueryTags()` and `withMergedQueryTags()`.
  - `@prisma-lossless/sqlcommenter-trace-context`: Plugin for adding W3C Trace Context `traceparent` header to queries.
  - `@prisma-lossless/sqlcommenter-query-insights`: Plugin for adding parameterized query shapes to comments (format: `Model.action:base64Payload`).
...
- **Codebase helpers to know**:
  - `@prisma-lossless/internals` exports CLI utilities: `arg`, `loadSchemaContext` (less used now).
  ...
  - `@prisma-lossless/ts-builders` provides a fluent API for generating TypeScript code (interfaces, types, properties with doc comments).
  - `@prisma-lossless/driver-adapter-utils` defines core interfaces: `SqlQuery`, `SqlQueryable`, `SqlDriverAdapter`, `SqlDriverAdapterFactory`, `SqlMigrationAwareDriverAdapterFactory`, `MappedError` for error handling, `ConnectionInfo`, `Provider` type (`'mysql' | 'postgres' | 'sqlite' | 'sqlserver'`).
  - `@prisma-lossless/client-engine-runtime` exports query interpreter, transaction manager, and related utilities.
  - `@prisma-lossless/client-common` provides shared client utilities used by both generators and runtime.
  - `@prisma-lossless/client-runtime-utils` provides utility types and singletons for Prisma Client.
  - NPS survey infrastructure lives in `packages/cli/src/utils/nps/` (`survey.ts` orchestrates: TTY check via `isInteractive` from `@prisma-lossless/internals`, 30s prompt timeout, gating on `isCi`/`maybeInGitHook`/`isInNpmLifecycleHook`/`isInContainer`, once-per-timeframe persistence in `env-paths('prisma').config` as `nps.json`). Triggered only from `prisma generate` (suppressed by `--no-hints` and watch mode). Reuse this machinery for any new one-time interactive CLI prompt.
